...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    expect({ name: 'Arya Stark', class: 'Rogue', phrases: ['Not today', 'A girl has no name.'] }).toMatchObject(getCharacter('Arya'));
    // Teste se a função retorna o objeto correto para o parâmetro 'Brienne',
    expect({ name: 'Brienne Tarth', class: 'Knight', phrases: ['Im No Lady, Your Grace.', 'I, Brienne Of Tarth, Sentence You To Die.'] }).toMatchObject(getCharacter('brieNNe'));
    // Teste se a função retorna o objeto correto para o parâmetro 'Melissandre',
    expect({ name: 'Melissandre', class: 'Necromancer', phrases: ['Death By Fire Is The Purest Death.', 'For The Night Is Dark And Full Of Terrors.'] }).toMatchObject(getCharacter('Melissandre'))
    ...
    expect(getCharacter('Joao Andrade')).toBeUndefined();
  });
});
